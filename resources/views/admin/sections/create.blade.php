...
@section('content')
@php
  $isEditing = isset($section);
@endphp
<section class="space-y-8">
  <header class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <div>
      <h1 class="text-2xl font-semibold tracking-tight">{{ $isEditing ? 'Edit section' : 'New section' }}</h1>
  ...
  <div class="card-surface">
    <form method="POST" action="{{ $isEditing ? route('admin.modules.sections.update', [$module, $section]) : route('admin.modules.sections.store', $module) }}" class="grid gap-6">
      @csrf
      @if($isEditing)
        @method('PUT')
      @endif
      ...
      <div class="flex items-center justify-end gap-3">
        ...
        <button class="btn btn-primary">{{ $isEditing ? 'Save changes' : 'Create section' }}</button>
      </div>
    </form>
  </div>
</section>
